  ...
  dropdownContainers.forEach(container => {
    ...
    if (trigger && menu) {
      trigger.addEventListener('click', (e) => {
        ...
        if (isTouch || trigger.getAttribute('href') === '#' || trigger.getAttribute('href') === 'javascript:void(0)') {
          e.preventDefault();
...
function initAccordions() {
  document.addEventListener('click', (e) => {
    ...
    if (!content || !content.classList.contains('accordion-content')) {
      const parentCard = header.closest('.border') || header.parentElement;
      content = parentCard ? parentCard.querySelector('.accordion-content') : null;
    }

    if (!content) return;

    const icon = header.querySelector('.accordion-icon') || header.querySelector('i.fa-chevron-down') || header.querySelector('i');
    const isHidden = content.classList.contains('hidden') || content.style.display === 'none';

    const container = header.closest('.space-y-4') || header.closest('section') || document;
    const allContents = container.querySelectorAll('.accordion-content');
    const allIcons = container.querySelectorAll('.accordion-header i, .accordion-icon');
